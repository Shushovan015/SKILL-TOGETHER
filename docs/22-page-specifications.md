...
| Onboarding | `/onboarding` | Configure Study Plan. | Learner | Authenticated, no active Enrollment for the selected track, or reconfiguring an owned Enrollment. | Learning Tracks, preferences. | Activate plan. | Save draft, cancel, replace existing plan. | Track loading, no tracks, capacity error. | Owner only. | Multi-step or stacked sections on mobile. | AC-ONBOARD-001 |
| My Tracks | `/tracks` | Manage active Learning Track enrollments. | Learner | Authenticated. | Learning Tracks and my Enrollments. | Continue track. | Add track, change plan, cancel plan, view roadmap. | Loading, no tracks, mutation error. | Owner only. | Track cards stack on mobile. | AC-TRACK-001 |
| Today Dashboard | `/today` | Show today's learning focus. | Learner | Authenticated. | Today tasks, the next planned session for every active track, progress, missed tasks, next assessment, partner summary. | Start or continue task. | Recover missed task, view week. | No enrollment, no task today, loading, auth error. | Owner only. | Today's tasks appear first; future sessions show their scheduled date. | AC-PLAN-001 |
...
| Exercise | `/lessons/:dailyTaskId/exercise` | Focus on guided or independent exercise. | Learner | Task belongs to learner. | Exercise prompt and expected evidence. | Complete lesson and return to My Tracks. | Return to lesson. | Draft saved, validation error; successful completion replaces the route with `/tracks`. | Owner only. | Single-column workspace on mobile. | AC-LESSON-002 |
